...
def makeSplusBPlot(workspace,hD,hSB,hB,hS,hDr,hBr,hSr,cat,options,dB=None,reduceRange=None,hC=None,MX=" "):
  ...
  h_axes = hD.Clone()
  h_axes.Reset()
  if options.doBands: h_axes.SetMaximum((hD.GetMaximum()+hD.GetBinError(hD.GetMaximumBin()))*1.2)
  ...
  if options.doBands:
    ...
    for ibin in range(h_axes.GetXaxis().GetFirst(),h_axes.GetNbinsX()+1):
      ...
      if options.doResonantBackground: bkgval = hC['nBins'].GetBinContent(ibin)      
  ...
  # Set pdf style
  if options.unblind:
    if options.doResonantBackground:
      leg.AddEntry(hC['pdfNBins'],"B model","l")
      leg.AddEntry(hB['pdfNBins'],"B + H","l")
    else:
      leg.AddEntry(hB['pdfNBins'],"B fit","l")
    leg.AddEntry(hSB['pdfNBins'],"B + H + HH","fl")
  else:
    if options.doResonantBackground:
      leg.AddEntry(hC['pdfNBins'],"B model","l")
      leg.AddEntry(hB['pdfNBins'],"B + H","l")
      ...
      leg.AddEntry(hB['pdfNBins'],"B fit","l")
    if options.signalScaler != 1:
      leg.AddEntry(hS['pdfNBins'],"HH model #scale[0.6]{(%s x SM)}"%options.signalScaler,"fl")
    else:
      leg.AddEntry(hS['pdfNBins'],"HH model","fl")
  if options.doBands:
  ...
  if options.unblind:
    ...
    hB['pdfNBins'].SetLineColor(4)
    hB['pdfNBins'].SetLineStyle(2)
    hB['pdfNBins'].Draw("Hist same c")
    if hC is not None:
      hC['pdfNBins'].SetLineWidth(2)
      hC['pdfNBins'].SetLineStyle(2)
      hC['pdfNBins'].SetLineColor(1)
      hC['pdfNBins'].Draw("Hist same c")
  else:
  ...
  h_axes_ratio.Reset()
  h_axes_ratio.SetMaximum(max((hDr.GetMaximum()+hDr.GetBinError(hDr.GetMaximumBin()))*1.5,hSr.GetMaximum()*1.4))
