...
def get_options():
  ...
  parser.add_option("--signalScaler", dest="signalScaler", default=1., type='float', help="Scaling term for signal histogram")
...
for cidx in range(len(cats)):
  ...
  h_bpdf2 = {'pdfNBins':b2pdf.createHistogram("h_b2_pdfNBins_%s"%c,_xvar,ROOT.RooFit.Binning(opt.pdfNBins,xvar.getMin(),xvar.getMax())),
             'nBins':b2pdf.createHistogram("h_b2_nBins_%s"%c,_xvar,ROOT.RooFit.Binning(opt.nBins,xvar.getMin(),xvar.getMax()))
            }
  # Calculate yields
  SB, B = sbpdf.expectedEvents(_xvar_argset), bpdf.expectedEvents(_xvar_argset)
  S = SB-B
  ...
  if opt.doBkgRenormalization:
    ...
    SBcorr = B
    normFactor_B, normFactor_SB = Bcorr/B, SBcorr/SB
    for h in h_sbpdf.values(): h.Scale(normFactor_SB)
    for h in h_bpdf.values(): h.Scale(normFactor_B)
    print("    * Yield for category: S = %.2f, B=%.2f"%(S,Bcorr))
  else: print("    * Yield for category: S = %.2f, B=%.2f"%(S,B))
  if opt.doResonantBackground:
    # w.var("r").setVal(0)
    w.var("r_singleHiggs").setVal(0)
    h_continuumpdf = {'pdfNBins':b2pdf.createHistogram("h_hcont_pdfNBins_%s"%c,_xvar,ROOT.RooFit.Binning(opt.pdfNBins,xvar.getMin(),xvar.getMax())),
               'nBins':b2pdf.createHistogram("h_hcont_nBins_%s"%c,_xvar,ROOT.RooFit.Binning(opt.nBins,xvar.getMin(),xvar.getMax()))}
    print("DEBUG, B values: ", b2pdf.expectedEvents(_xvar_argset))
    w.var("r_singleHiggs").setVal(1)
  else:
    h_continuumpdf = h_bpdf
  # Extract signal pdf
  ...
  if opt.nBins != xvar_range:
    ...
    if opt.doResonantBackground:
      for h in h_continuumpdf.itervalues(): h.Scale(float(xvar_range)/opt.nBins)
  if opt.signalScaler != 1.:
    for h in h_spdf.itervalues(): h.Scale(opt.signalScaler)
  ...
  if opt.doResonantBackground: 
      h_bpdf_ratio = h_bpdf['pdfNBins']-h_continuumpdf['pdfNBins']
      h_spdf_ratio = h_sbpdf['pdfNBins']-h_continuumpdf['pdfNBins']
  ...
  for ibin in range(1,h_data.GetNbinsX()+1):
    ...
    bkgvalall = h_bpdf['nBins'].GetBinContent(ibin)
    bkgval = h_continuumpdf['nBins'].GetBinContent(ibin)
    if opt.doResonantBackground: bkgval = h_continuumpdf['nBins'].GetBinContent(ibin)    
  ...
  if not opt.doResonantBackground: h_continuumpdf = None
  ...
  h_data.Delete()
  h_data_ratio.Delete()
  for h in h_sbpdf.values(): h.Delete()
  for h in h_bpdf.values(): h.Delete()
  h_bpdf_ratio.Delete()
  for h in h_spdf.values(): h.Delete()
  h_spdf_ratio.Delete()
  if opt.doCatWeights:
    h_wdata.Delete()
    h_wdata_ratio.Delete()
    for h in h_wsbpdf.values(): h.Delete()
    for h in h_wbpdf.values(): h.Delete()
    h_wbpdf_ratio.Delete()
    for h in h_wspdf.values(): h.Delete()
