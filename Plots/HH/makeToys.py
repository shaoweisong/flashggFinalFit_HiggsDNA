...
if opt.dropResonantBkg: 
  setParamStr += "r_singleHiggs=0,"
  setParam0Str += "r_singleHiggs=0,"
for p,v in poi_bf.items(): 
  setParamStr += "%s=%.3f,"%(p,v)
  setParam0Str += "%s=0,"%p
print(" --> Setting parameters: %s"%setParamStr)
...
if opt.batch == 'IC':
  ...
  for itoy in range(0,opt.nToys):
    ...
    fsub.write("#!/bin/bash\n\n")
    fsub.write("cd %s/src/flashggFinalFit/Plots/HH/SplusBModels%s/toys\n\n"%(os.environ['CMSSW_BASE'],opt.ext))
  ...
  print(" --> Generate command: %s"%gen_cmd)
  if opt.loadSnapshot is not None: gen_cmd += " --snapshotName %s"%opt.loadSnapshot
  fsub.write("%s\n\n"%gen_cmd)
  # Fit cmd
  fsub.write("#Fit command\n")
  ...
  fcondor.write("output                = sub_toys.$(ClusterId).$(ProcId).out\n")
  fcondor.write("error                 = sub_toys.$(ClusterId).$(ProcId).err\n")
  fcondor.write("log                   = sub_toys.$(ClusterId).log\n\n")
  ...
  fcondor.write("queue %s"%opt.nToys)
  fcondor.close()
  ...
  if not opt.dryRun: 
    if os.environ['PWD'].startswith("/eos"):
      os.system("cd ./SplusBModels%s/toys/jobs; condor_submit -spool sub_toys.sub; cd ../../.."%opt.ext)
    else:
      os.system("cd ./SplusBModels%s/toys/jobs; condor_submit sub_toys.sub; cd ../../.."%opt.ext)
  else: print(" --> [DRY-RUN] jobs have not been submitted")  
